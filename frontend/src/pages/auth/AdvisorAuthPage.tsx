...
import {
  Building2, ChevronLeft, User, Mail, Phone, Lock, Eye, EyeOff,
...
export default function AdvisorAuthPage() {
  ...
  // Already logged in as advisor → go to dashboard
  useEffect(() => {
    if (session.getType() === 'advisor') navigate('/asesores', { replace: true })
  }, [navigate])
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const r = await API.post('/chat/advisors/login', { email: loginEmail.trim(), password: loginPass })
      // Load full profile to get name
      const full = await API.get('/chat/advisors/me/full', { headers: { Authorization: `Bearer ${r.data.token}` } })
      session.clear()
      session.set({ type: 'advisor', token: r.data.token, name: full.data.name, email: full.data.email })
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setRegLoading(true)
    try {
      await API.post('/chat/advisors/register', { name: regName.trim(), email: regEmail.trim(), phone: regPhone.trim() || null, password: regPass })
      setRegDone(true)
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail
  ...
  return (
    <div className="min-h-screen relative flex flex-col overflow-hidden" style={{ background: 'linear-gradient(135deg, #e8f0fe 0%, #dce8fb 40%, #cfe0f8 100%)' }}>
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] rounded-full opacity-30 pointer-events-none" style={{ background: 'radial-gradient(circle, #93c5fd 0%, #60a5fa 60%, transparent 100%)', transform: 'translate(-30%, 30%)' }} />
      <div className="absolute top-0 right-0 w-[350px] h-[350px] rounded-full opacity-20 pointer-events-none" style={{ background: 'radial-gradient(circle, #a5b4fc 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-4 py-10">
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border border-white/60 w-full max-w-[520px] flex flex-col overflow-hidden">
            ...
            <form onSubmit={handleLogin} className="flex flex-col flex-1 overflow-hidden">
              <div className="flex-1 overflow-y-auto px-8 pt-6 pb-2 space-y-4">
                <div className="text-center mb-5">
                  ...
                  <p className="text-sm text-slate-500 mt-1.5">Gestiona tus leads y oportunidades</p>
                </div>
                <div>
            ...
            <form onSubmit={handleRegister} className="flex flex-col flex-1 overflow-hidden">
              <div className="flex-1 overflow-y-auto px-8 pt-6 pb-2 space-y-4">
                <div className="text-center mb-5">
                  ...
                  <p className="text-sm text-slate-500 mt-1.5">Solicita acceso al portal de asesores</p>
                </div>
                {([
                ...
                ] as { label: string; value: string; setter: (v: string) => void; type: string; placeholder: string; Icon: typeof Mail }[]).map(({ label, value, setter, type, placeholder, Icon }) => (
                  <div key={label}>
          ...
          {tab === 'register' && regDone && (
            <div className="flex flex-col flex-1 items-center justify-center px-8 pb-8 text-center space-y-4">
              ...
              <button onClick={() => { setTab('login'); setRegDone(false) }}
                className="w-full py-3 rounded-xl border border-slate-200 bg-white text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors">
