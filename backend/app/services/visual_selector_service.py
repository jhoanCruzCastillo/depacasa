...
class VisualSelectorManager:
    ...
    async def capture_and_analyze_rect(
        ...
        system_msg = (
            ...
            "Analizas imágenes de páginas web y generas plantillas de extracción JSON.\n\n"
            "NOMBRES DE CAMPO ESTÁNDAR para catálogos de proyectos:\n"
            "- nombre → nombre del proyecto\n"
            "- estado_del_proyecto → etiqueta de estado (EN CONSTRUCCIÓN, ENTREGA INMEDIATA, LANZAMIENTO...)\n"
            "- ubicacion → distrito o ciudad\n"
            "- precio_desde → precio mínimo (number)\n"
            "- imagen → foto principal  → is_image:true, extract_attr:'src'\n"
            "- url_propiedad → enlace al detalle → is_child_url:true\n"
            "- descripcion → texto descriptivo\n"
            "- dormitorios → número de habitaciones\n"
            "- m2 → metros cuadrados\n"
            "- modelo → nombre del modelo o tipología\n\n"
            "REGLAS:\n"
            "- Si el área muestra elementos que se repiten (cards/tarjetas) → section_type:'catalog'\n"
            "- catalog_type puede ser: proyectos, imagenes, areas_comunes, propiedades, zonas_cercanas\n"
            "- card_selector = selector CSS del elemento raíz de cada card (relativo al documento)\n"
            "- field.selector = selector RELATIVO al card (no al documento)\n"
            "- needs_hover = true si el hover revelaba contenido que normalmente está oculto\n"
            "- Extrae TODOS los campos visibles en la imagen\n\n"
            "Devuelve SOLO el JSON, sin texto adicional:\n"
            '{"section_type":"catalog","catalog_type":"proyectos","needs_hover":false,'
            '"card_selector":"...","fields":['
            '{"name":"nombre","selector":".title","type":"text","is_image":false,"is_child_url":false,"is_list":false,"extract_attr":null}'
            '],"confidence":0.9}'
        )
